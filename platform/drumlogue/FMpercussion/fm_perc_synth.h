...
fast_inline void apply_resonant_morph(resonant_synth_t * res, float morph, uint8_t mode) {
  ...
  switch (mode) {
    ...
    {
      float fc = 1000.0f;
      float width = 0.1f + morph * 2.0f;  // Width multiplier (TODO: pass to resonance calc)
      (void)width; // Width affects Q internally
      resonant_synth_set_center(res, all_voices, vdupq_n_f32(fc));
    ...
    case 4:  // Peak - morph controls both frequency and gain
    {
      ...
      float gain = 1.0f + morph * 3.0f;     // 1-4x gain (TODO: apply in process function)
      (void)gain;
      resonant_synth_set_center(res, all_voices, vdupq_n_f32(fc));
