/**
 *  @file lfo_enhanced.h
 *  @brief Enhanced LFO system with bipolar modulation and phase independence
 */
...
#define LFO_TARGET_LFO2_PHASE 4  // LFO1 -> LFO2 phase
#define LFO_TARGET_LFO1_PHASE 5  // LFO2 -> LFO1 phase

// Phase offset to prevent cancellation (90° = 0.25 cycle)
#define LFO_PHASE_OFFSET 0.25f

/**
 * Enhanced LFO state with phase independence
 */
typedef struct {
    // Primary LFOs
    ...
    int32_t target1;          // 0-5 LFO1 target
    int32_t target2;          // 0-5 LFO2 target
    ...
    uint8_t mod_matrix[2][2];  // [source][target] connection matrix
} lfo_enhanced_t;
...
fast_inline void lfo_enhanced_init(lfo_enhanced_t* lfo) {
    // Set phases with 90° offset
    lfo->phase1 = vdupq_n_f32(0.0f);
    lfo->phase2 = vdupq_n_f32(LFO_PHASE_OFFSET);
    
    lfo->shape_combo = 0;  // Both triangle
    lfo->target1 = LFO_TARGET_NONE;
    lfo->target2 = LFO_TARGET_NONE;
    lfo->depth1 = vdupq_n_f32(0.0f);
    lfo->depth2 = vdupq_n_f32(0.0f);
    lfo->rate1 = vdupq_n_f32(0.01f);  // Default slow rate
    lfo->rate2 = vdupq_n_f32(0.01f);
    
    // Initialize modulation matrix (no connections)
    lfo->mod_matrix[0][0] = 0;  // LFO1 -> LFO1 (invalid)
    lfo->mod_matrix[0][1] = 0;  // LFO1 -> LFO2
    lfo->mod_matrix[1][0] = 0;  // LFO2 -> LFO1
    lfo->mod_matrix[1][1] = 0;  // LFO2 -> LFO2 (invalid)
}

/**
 * Update LFO parameters with circular reference protection
 */
fast_inline void lfo_enhanced_update(lfo_enhanced_t* lfo,
    ...
    // Check for circular references before updating targets
    uint8_t new_target1 = target1;
    uint8_t new_target2 = target2;
    
    // Detect and prevent LFO1 <-> LFO2 circular modulation
    if ((new_target1 == LFO_TARGET_LFO2_PHASE && 
         new_target2 == LFO_TARGET_LFO1_PHASE) ||
        (new_target2 == LFO_TARGET_LFO1_PHASE && 
         new_target1 == LFO_TARGET_LFO2_PHASE)) {
        // Circular reference detected! Disable both LFO-to-LFO modulations
    ...
    // Convert percentage to bipolar depth (-1.0 to 1.0)
    float depth1 = (depth1_percent / 50.0f) - 1.0f;  // 0% = -1.0, 50% = 0.0, 100% = 1.0
    float depth2 = (depth2_percent / 50.0f) - 1.0f;
