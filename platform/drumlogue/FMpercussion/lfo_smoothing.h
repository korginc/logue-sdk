...
fast_inline void lfo_smoother_set_rate(lfo_smoother_t* sm,
    ...
    if (lfo_num == 0) {
        ...
        sm->rate_step1 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                   vdivq_f32(diff, vdupq_n_f32(LFO_SMOOTH_FRAMES)),
                                   sm->rate_step1);

        // Set target and start ramp
        sm->target_rate1 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                     new_rate_vec,
                                     sm->target_rate1);
        ...
        sm->rate_step2 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                   vdivq_f32(diff, vdupq_n_f32(LFO_SMOOTH_FRAMES)),
                                   sm->rate_step2);

        sm->target_rate2 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                     new_rate_vec,
                                     sm->target_rate2);
...
fast_inline void lfo_smoother_set_depth(lfo_smoother_t* sm,
    ...
    if (lfo_num == 0) {
        ...
        sm->depth_step1 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                    vdivq_f32(diff, vdupq_n_f32(LFO_SMOOTH_FRAMES)),
                                    sm->depth_step1);

        sm->target_depth1 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                      new_depth_vec,
                                      sm->target_depth1);
        ...
        sm->depth_step2 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                    vdivq_f32(diff, vdupq_n_f32(LFO_SMOOTH_FRAMES)),
                                    sm->depth_step2);

        sm->target_depth2 = vbslq_f32(vreinterpretq_f32_u32(voice_mask),
                                      new_depth_vec,
                                      sm->target_depth2);
...
fast_inline void lfo_smoother_process(lfo_smoother_t* sm) {
    ...
    sm->current_rate1 = vbslq_f32(vreinterpretq_f32_u32(rate1_active),
    ...
    sm->current_rate1 = vbslq_f32(vreinterpretq_f32_u32(rate1_done),
                                  sm->target_rate1,
                                  sm->current_rate1);

    // Smooth rate2 (similar)
    uint32x4_t rate2_active = vcgtq_u32(sm->rate_ramp2_counter, vdupq_n_u32(0));
    sm->current_rate2 = vbslq_f32(vreinterpretq_f32_u32(rate2_active),
    ...
    sm->current_rate2 = vbslq_f32(vreinterpretq_f32_u32(rate2_done),
                                  sm->target_rate2,
                                  sm->current_rate2);

    // Smooth depth1 (bipolar)
    uint32x4_t depth1_active = vcgtq_u32(sm->depth_ramp1_counter, vdupq_n_u32(0));
    sm->current_depth1 = vbslq_f32(vreinterpretq_f32_u32(depth1_active),
    ...
    sm->current_depth1 = vbslq_f32(vreinterpretq_f32_u32(depth1_done),
                                   sm->target_depth1,
                                   sm->current_depth1);

    // Smooth depth2
    uint32x4_t depth2_active = vcgtq_u32(sm->depth_ramp2_counter, vdupq_n_u32(0));
    sm->current_depth2 = vbslq_f32(vreinterpretq_f32_u32(depth2_active),
    ...
    sm->current_depth2 = vbslq_f32(vreinterpretq_f32_u32(depth2_done),
                                   sm->target_depth2,
                                   sm->current_depth2);
