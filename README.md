...
- one-command **`make websim`** from the repo root to build and open any unit;
- three UI shells — a keyboard oscillator, a sample-through effect, and a kaoss X/Y pad — with
  live parameter sliders, a scope, and default-on playback;
- a **headless render harness** (`make render`) + checker for CI audio smoke-tests.
